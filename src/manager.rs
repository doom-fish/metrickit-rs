...
use crate::ffi;
use crate::metrics::{DiagnosticPayload, MetricPayload};
...
impl MetricManager {
    ...
    pub fn past_payloads(&self) -> Result<Vec<MetricPayload>, MetricKitError> {
        let ptr = unsafe { ffi::mx_metric_manager_past_payloads_json() };
        if ptr.is_null() {
            return Ok(Vec::new());
        }
        decode_json(ptr)
    ...
    pub fn past_diagnostic_payloads(&self) -> Result<Vec<DiagnosticPayload>, MetricKitError> {
        let ptr = unsafe { ffi::mx_metric_manager_past_diagnostic_payloads_json() };
        if ptr.is_null() {
            return Ok(Vec::new());
        }
        decode_json(ptr)
        ...
        D: MetricSubscriberDelegate + 'static,
    {
        ...
        let status = unsafe {
            ffi::mx_metric_manager_add_subscriber(
...
impl Drop for MetricSubscription {
    fn drop(&mut self) {
        ...
        unsafe {
            ffi::mx_metric_manager_remove_subscriber(self.raw);
